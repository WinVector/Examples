...
```{r}
characterize_dists <- function(d, subtitle = NULL) {
  ...
  # Please see: https://win-vector.com/2020/10/29/a-single-parameter-family-characterizing-probability-model-performance/
...
mk_normal_example <- function(mean_pos, mean_neg, common_sd) {
  eval_points <- seq(-10, 10, by = 0.01)
  sigmoid <- function(x) { 1 / (1 + exp(-x)) }
  data.frame(
    model_prediction = sigmoid(eval_points),
    positive_density = dnorm(eval_points, mean = mean_pos, sd = common_sd),
    negative_density = dnorm(eval_points, mean = mean_neg, sd = common_sd))
}

d_normal <- mk_normal_example(mean_pos = 0, mean_neg = -1.5, common_sd = 1)
```
