...
def theoretical_payout(red_remaining: int, black_remaining: int):
    return 2**(red_remaining + black_remaining) / comb(red_remaining + black_remaining, red_remaining, exact=True)
    ...
    for i in range(len(is_red)):
        ...
        if verbose:
            ...
            if bet_red > 0:
                ...
                bet_color = 'black'
            print(f"step({i}, color: {'red' if is_red[i] else 'black'}) stake: {stake}, bet {bet_color}: {bet_red + bet_black}")
        # derive outcome
        stake = stake - (bet_red + bet_black)
        if is_red[i]:
            stake = stake + 2 * bet_red
...
def basic_bet_rules(
        holdings: int, 
        n_black_remaining: int, n_red_remaining: int, 
        satiation_point: int) -> Optional[int]:
...
def basic_bet_strategy(
        holdings: int, 
        n_black_remaining: int, n_red_remaining: int, 
        satiation_point: int) -> int:
    """return signed bet (positive black, negative red)"""
    if n_red_remaining > n_black_remaining:
        return -basic_bet_strategy(holdings, n_red_remaining, n_black_remaining, satiation_point)
    bet = basic_bet_rules(holdings, n_black_remaining, n_red_remaining, satiation_point)
    if bet is not None:
        return bet
    v = int(max(0, min(holdings,
                np.round(holdings * (n_black_remaining - n_red_remaining) / (n_black_remaining + n_red_remaining)))))
    if (v > 0) and (v >= holdings) and (n_black_remaining > 0) and (n_red_remaining > 0):
...
def _bet_amt_min_mean(
        holdings: int, 
        n_black_remaining: int, n_red_remaining: int, 
        satiation_point: int) -> Tuple[int, int, float]:
    ...
    for black_bet in range(holdings + 1):
        a = bet_amt_min_mean(holdings + black_bet, n_black_remaining - 1, n_red_remaining, satiation_point)
        b = bet_amt_min_mean(holdings - black_bet, n_black_remaining, n_red_remaining - 1, satiation_point)
        min_return = int(min(a[1], b[1]))
        mean_return = (
            (n_black_remaining / (n_black_remaining + n_red_remaining)) * a[2]
            + (n_red_remaining / (n_black_remaining + n_red_remaining)) * b[2]
        )
        if((best_black_bet is None) 
...
def bet_amt_min_mean(
    ...
    if n_black_remaining == n_red_remaining:
        sub = bet_amt_min_mean(holdings, n_black_remaining, n_red_remaining - 1, satiation_point)
        return (0, sub[1], sub[2])
    # have n_black_remaining > n_red_remaining
    if n_red_remaining <= 0:
        return (holdings, holdings * 2 ** n_black_remaining, holdings * 2 ** n_black_remaining)
    if holdings > satiation_point:  # early exit to prevent tracking too many possible holdings
        return (0, holdings, holdings)  # artificial "don't bet" as not needed to establish minimum
    return _bet_amt_min_mean(holdings, n_black_remaining, n_red_remaining, satiation_point)
...
def dynprog_bet_strategy(
    ...
    """return signed bet (positive black, negative red)"""
    if n_red_remaining > n_black_remaining:
        return -dynprog_bet_strategy(holdings, n_red_remaining, n_black_remaining, satiation_point)
    bet = basic_bet_rules(holdings, n_black_remaining, n_red_remaining, satiation_point)
    if bet is not None:
        return bet
    if holdings >= satiation_point:  # for large values, bet near ideal value
        ...
        v = bet_amt_min_mean(holdings, n_black_remaining, n_red_remaining, satiation_point)[0]
    if (v > 0) and (v >= holdings) and (n_black_remaining > 0) and (n_red_remaining > 0):
        v = v - 1  # don't trade entire holdings into uncertainty
    return v
