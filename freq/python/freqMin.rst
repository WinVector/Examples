...
.. code:: python
    ...
    explicitGradientClearingEqns = True # we get the same solution with this on or off, so just an issue of helps/hurts
    ...
    def solveForK(k):
       ...
       if explicitGradientClearingEqns:
           for phi in phis:
               eqns.extend(prepareClearingEqns(p,phis,sympy.diff(poly,p)))
