...
However the existence of so many different algorithms, and their associated software,
...
There are various reasons why you might want to port a model. We once had a client
...
want to apply it into a production system, you may find that the software that you
...
So you need to fit the model one place, then transfer it to your preferred deployment framework.
...
often carry their training data and fitting diagnostic data around with them. This
can get fairly heavyweight when you have a lot of data. While there are ways to
turn off saving the training data, or to delete the artifacts, you can also just create
a fresh version of the model, with lighter artifacts.
...
If $U$ is full rank, then solving $U \mathbf{b} = Y$ for $\mathbf{b}$ will uniquely recover the coeffients of the model $m$. Remember, it *doesn't matter* whether the rows of $U$ are realistic with respect to the original problem domain; we're not doing statistics anymore,
we're just doing linear algebra. So a handy candidate for $U$ is
...
* [Port a quantile regression model to an OLS framework](https://github.com/WinVector/scratch/blob/main/linear_model_transfer/linear_model_transfer_linear_logical.md) 
...
* [Porting linear models with categorical variables](https://github.com/WinVector/scratch/blob/main/linear_model_transfer/linear_model_transfer_categorical.md) 
...
where $f()$ is the so-called *link function*, and $f^{-1}()$ is the *inverse link function*. 
...
Note that since you are using the actual generalized linear model fitter to re-fit a model to $U$, you want to make sure that you construct $Y$ using $y_{pred}$ (in R, the "response"), **NOT** $f_{ypred}$ (in R, the "link"). For logistic regression, this means you construct $Y$ from your original model's probability predictions, not the logit of the probabilities.
...
The below notebook shows an extended example of transferring the coefficients from a L2-regularized logistic regression model (fit with the `glmnet` package) to a "standard" (`glm`) logistic regression model in R. I've noted some issues and caveats that will also be relevant to Python users.

* [Porting a logistic regression model from one framework to another](https://github.com/WinVector/scratch/blob/main/linear_model_transfer/logistic_model_transfer.md) TODO add link
