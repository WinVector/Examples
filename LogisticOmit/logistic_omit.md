...
First let’s start with a data example in
...
``` r
d <- merge(x_frame, omitted_frame, by = c())
d$wt = d$wt.x * d$wt.y
d$wt.x <- NULL
d$wt.y <- NULL
d$wt <- d$wt / sum(d$wt)
...
Now, let’s define a few common constants: the Euler constant, `pi`, and
`e`.
...
For our example call our outcome (or dependent variable) `y_linear`. We
say that it is exactly the following linear combination of the `x` and
`omitted` variables, plus a constant.
...
Let’s augment our data to recover the model coefficients from the
frequency a new outcome variable, called `y_observed` takes on the
values `TRUE` and `FALSE`. We describe this sort of encoding strategy to
[replicate linear and logistic
...
this model. So let’s take a moment and derive our data.
...
variable `x`, even under the new data set and new row distribution.
...
Notice the new `x` coefficient is no where near the value we saw before.
...
> is mean zero, symmetric, and uncorrelated with the model explanitory
> variables. This differs from the situation for linear models.

The good way of interpreting logistic experiment is:

> For a logistic model: the correct inference for a given explanatory
> variable often depends on what other explanatory variables are
> present.
...
of other explanatory variables is. This is not the case for logistic
...
only by changes in `omitted`.

Without the extra variable `omitted` we can’t tell the joined pairs
apart, and we are forced to use compromise effect estimates.

In the linear space we see the same amount of horizontal or `y_linear`
perturbation in the `x = -2` and `x = 1` groups. However, we see
radically different amounts of perturbation in vertical or
`y_probability` direction (due to different slops in different regions
of the logistic curve). The uncorrelated variable `omitted` has
different effects in probability prediction depending on the values of
any other explanatory variables. This is a common observation in
logistic regression: you can’t tell if a variable and coefficient have
large or small effect without knowing the typical values of the
...
the data having fundamentally different models. The possible source of
surprise is: linear models avoid this in the special case of independent
variables.

For logistic models to get the mean-effect of a variable we must apply
the effect to each data row and then average. We can not appeal to
linearity to say a mean-zero independent effect won’t change average
outcomes. This is because, while a logistic regression has linear
components, it is not (due to its use of the non-linear inverse link of
`sigmoid()`) itself a linear function.

Some care has to be taken in moving inferred logistic coefficients out
