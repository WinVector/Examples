---
title: "Overcoming Omitted Variable Bias by Solving for Hidden Data"
...
This is a known problem with known mitigations:
  ...
  * Zhang, Zhiwei (2009) "Estimating a Marginal Causal Odds Ratio Subject to Confounding", Communications in Statistics - Theory and Methods, 38:3, 309 — 321.
...
Notice experimenter 1 got a *much* too small estimate of the `x1` coefficient. From experimenter 1's point of view, the effect of the omitted variable `x2` is making `x1` hard to correctly infer.
...
Each experimenter knows a lot about the data. They known the distribution of their explanatory variable, and even the joint distribution of their explanatory and the dependent and outcome data. Assuming the two explanatory variables are independent, they even know the joint distribution of the explanatory variables.
...
Putting this all together we get a joint estimate of the complete `proportion` vector.
...
We use linear algebra to solve for an example data distribution estimate (with illegal negative entries!).
...
We have a single dimensional null space, which is the direction different possible solutions vary in.
...
All valid solutions are of the form `v + z * ns` for scalars `z`. In fact all solutions are some interval of `z` values. Let's solve for them.
...
It is convenient to pick a distinguished or "best guess" solution. In our case here it doesn't matter much, as both our extreme solutions have nearly identical logistic regression inferences. However if we do want to pick a single guess at the data pre-image the usual method is to pick the maximum entropy pre-image distribution. 

This is just a simple principle: prefer flat distributions until one have evidence against them. This modeling technique is itself strongly related to logistic regression modeling. Or one can say this is a bit opportunistic, we have an under-conditioned problem so we add an arbitrary convex criterion to pick a solution. Roughly we don't want to over-sell the maximum entropy pick, as we are already enforcing a lot of our sensible desiderata with the linear constraints.
