...
All valid solutions are of the form `v + z * ns` for scalars `z`. In fact all solutions are some interval of `z` values. We can solve for this interval.
...
We have the direction we are varying (`ns`) before, it is `test_vec`! So we know the actual solution is orthogonal to `ns`. Some calculus will show us in our case the entropy is maximized where the gradient is zero, and that this gradient being zero is the same condition as our distribution being orthogonal to `ns`. So the maximum entropy condition is enforcing the "no interactions" invariant we commented on earlier.

The funny thing is, we don't have to know exactly what the maximum entropy objective was doing to actually benefit from it. It tends to be a helpful objective in modeling.
