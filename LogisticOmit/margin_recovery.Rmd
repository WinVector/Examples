...
```{r}
design_names <- expand.grid(x1 = c('0', '1'), x2 = c('0', '1'), y = '*', stringsAsFactors = FALSE)
design_transform <- matrix(data=0, nrow = nrow(design_names), ncol = nrow(detailed_names))
colnames(design_transform) <- paste0("n(", apply(detailed_names, 1, paste, collapse = ","), ")")
rownames(design_transform) <- paste0("n(", apply(design_names, 1, paste, collapse = ","), ")")
for (row_index in seq(nrow(design_transform))) {
  for (col_index in seq(ncol(design_transform))) {
    if(sum(detailed_names[col_index, ] == design_names[row_index, ]) == 2) {
      design_transform[row_index, col_index] = 1
    }
  }
}

knitr::kable(design_transform)
```
...
```{r}
opt <- lp("min", c(1, 1, 1, 1), cbind(ns, -ns), ">=", -v)
