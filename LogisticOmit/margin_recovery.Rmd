---
title: "Overcoming Omitted Variable Bias by Pooling Data"
...
detailed_names <- detailed_names[order(detailed_names$y, detailed_names$x2, detailed_names$x1), , drop = FALSE]
...
margin_frame$proportion <- as.numeric(margin_transform %*% detailed_frame$proportion)

knitr::kable(margin_frame)
```

The margin frame describes how the detailed experiment is marginalized or censored down to what different experimenters that have access to only some of the explanatory variables see. Finding a good pre-image of data under this transform is how we will solve our problem.

We are going to consider the original detailed data as "unobserved", and parts of the marginalized results as what is observed by two experimenters.
...
dx <- margin_frame[margin_frame$y == '*', , drop = FALSE]
...
stopifnot(all(dx[, c("x1", "x2")] == dxe[, c("x1", "x2")]))

knitr::kable(dxe)
```

Notice `dxe` is build only from `dx1` and `dx2` (plus the assumed independence of `x1` and `x2`).
...
At this point we have inferred the `pX1` and `pX2` parameters from the observed data.
...
v <- as.numeric(MASS::ginv(margin_transform) %*% estimated_proportions)
...
stopifnot(z_opt >= 0)
stopifnot(z_opt <= 1)
detailed_frame["maxent_dist"] <- z_opt * detailed_frame$recovered_1 + (1 - z_opt) * detailed_frame$recovered_2
...
The point is: by pooling their observations the two researchers can recover a very good estimate of a joint analysis that neither of them performed. The strategy we used is: try to estimate plausible pre-images of the data that formed the observations that they saw, and then analyze that. This in fact gives us a method to invert the bias introduced by the omitted variable observations.
