...
Let's specify the probabilities each variable is `1`, which we are specifying as independent with the following expected values.
...
What we are going to do is: simulate experimenter 1 and experimenter 2 working together to try and recover a joint estimate of the `x1` and `x2` coefficients.

First we characterize all pre-images of the pooled marginal information. These are all of the form of a pre-image of the observed `margins` vector plus any elements of the marginalization processes' null-space (i.e. changes in data that are not respected by the summation process).
...
And, in this case, it turns out each of our extreme solutions recovers essentially the same logistic regression solution! And this matches the unobserved detailed solution!
...
In the real world the two experimenters at best would be looking at marginalizations of different draws of related data. So we would not have exact matches we can invert- but instead would have to guess low-norm pre-images of the data. And, as always, as we are now introducing a lot of unobserved data we could got to Bayesian graphical model methods to integreate this out (instead of proposing a specific pointwise heuristic as we did here).
