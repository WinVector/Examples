...
I would like to illustrate a way which omitted variables interfere in [logistic regression](https://en.wikipedia.org/wiki/Logistic_regression) inference. These effects are different than what is seen in [linear regression](https://en.wikipedia.org/wiki/Linear_regression), and possibly different than some expectations or intuitions.
...
For our first example we take the cross-product of there data frames to get every combination of variable values, and their relative proportions (or weights) in the joined data frame.
...
> For a logistic regression model: an omitted explanatory variable can bias other coefficient estimates. This even when the omitted explanatory variable is mean zero, symmetric, and uncorrelated with the other model explanitory variables. This differs from the situation for linear models.
...
What are your opinions/experience? Some questions I feel are relevant include:
  ...
  * What is the correct value of the `x`-coefficient in the logistic regressions? `3.1415`, `1.8522`, both, or neither?
