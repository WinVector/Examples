...
First let's start with a data example in [`R`](https://www.r-project.org).
...
`x_frame` is `data.frame` with a single variable called `x`, and an example weight called `wt`.
...
Now, let's define a few common constants: the Euler constant, `pi`, and `e`.
...
For our example call our outcome (or dependent variable) `y_linear`. We say that it is exactly the following linear combination of the `x` and `omitted` variables, plus a constant.
...
Let's augment our data to recover the model coefficients from the frequency a new outcome variable, called `y_observed` takes on the values `TRUE` and `FALSE`. We describe this sort of encoding strategy to [replicate linear and logistic models](https://win-vector.com/2019/07/03/replicating-a-linear-model/) (which can simplify steps in many data science projects). How this example arises isn't critical, we want to investigate the properties of this model. So let's take a moment and derive our data.
...
First we confirm `omitted` is mean zero and uncorrelated with our variable `x`, even under the new data set and new row distribution.
...
> For a logistic model: an omitted explanatory variable can bias coefficient estimates. This even when the omitted explanatory variable is mean zero, symmetric, and uncorrelated with the model explanitory variables. This differs from the situation for linear models.

The good way of interpreting logistic experiment is:

> For a logistic model: the correct inference for a given explanatory variable often depends on what other explanatory variables are present.

That is: we didn't get a wrong inference. We just got a different one- as we are inferring in a different situation. The fallacy was thinking a change in variable value has the same effect no matter what the values of other explanatory variables is. This is not the case for logistic regression, due to the non-linear shape of the logistic curve.
...
In the above diagram we have the `sigmoid()`, or logistic curve. The horizontal axis is the linear or "link space" for predictions and the vertical axis is the probability or response space for predictions. The curve is the transform the logistic regression's linear or link prediction is run through to get probabilities or responses. On this curve we have added as dots the four different combinations of values for `x` and `omitted` in our data set. The dots attached by lines differ only by changes in `omitted`.

Without the extra variable `omitted` we can't tell the joined pairs apart, and we are forced to use compromise effect estimates.

In the linear space we see the same amount of horizontal or `y_linear` perturbation in the `x = -2` and `x = 1` groups. However, we see radically different amounts of perturbation in vertical or `y_probability` direction (due to different slops in different regions of the logistic curve). The uncorrelated variable `omitted` has different effects in probability prediction depending on the values of any other explanatory variables. This is a common observation in logistic regression: you can't tell if a variable and coefficient have large or small effect without knowing the typical values of the complementary explanatory variables.

## My Interpretation

You get different estimates for variables depending on what other variables are present in a logistic model. This looks a lot like omitted variable bias, but it is also interpretable as different column-views of the data having fundamentally different models. The possible source of surprise is: linear models avoid this in the special case of independent variables.

For logistic models to get the mean-effect of a variable we must apply the effect to each data row and then average. We can not appeal to linearity to say a mean-zero independent effect won't change average outcomes. This is because, while a logistic regression has linear components, it is not (due to its use of the non-linear inverse link of `sigmoid()`) itself a linear function.

Some care has to be taken in moving inferred logistic coefficients out of their surrounding model context.
...
What are your opinions to the nature of the effect? 
  ...
  * Is it important? 
  * Is it intrinsic to the modeling process, or introduced by attempted interpretation?
