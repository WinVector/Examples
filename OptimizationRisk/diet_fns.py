...
# nutritional information is per dollar, not per unit
...
nutrients.columns = ["requirement", "lower bound"]
...
data.columns = ["item", "unit", "price"] + list(nutrients["requirement"])


def solve_diet(
    *, data: pd.DataFrame, nutrients: pd.DataFrame, risk_scale: float = 0
) -> np.ndarray:
    n_vars = data.shape[0]
    # estimate sqrt(q) as (last_q + q) / (2 sqrt(q)) (a common approximation)
    prev_last_q = None
    last_q = None
    for pass_number in range(1000):
        w_variable = cp.Variable(n_vars)
        price = np.full(
            (n_vars,), 1.0, dtype=float
        )  # nutritional information is per dollar, not per unit
        objective = cp.Minimize(cp.sum(cp.multiply(w_variable, price)))
        constraints = [w_variable >= 0]
        for j in range(nutrients.shape[0]):
            v = np.array(data.loc[:, nutrients.loc[j, "requirement"]], dtype=float)
            b = nutrients.loc[j, "lower bound"]
            constraints.append(cp.sum(cp.multiply(w_variable, v)) >= b)
            if (risk_scale > 0) and (last_q is not None):
                cv = nutrients.loc[j, "coefficient of variation"]
                q = cp.quad_form(w_variable, np.diag((v * cv) ** 2))
                sqrt_q_est = (1e-9 + last_q[j] + q) / (1e-6 + 2 * np.sqrt(last_q[j]))
                constraints.append(
                    risk_scale * sqrt_q_est <= cp.sum(cp.multiply(w_variable, v)) - b
                )
        prob = cp.Problem(objective, constraints)
        prob.solve()
        if risk_scale > 0:
            prev_last_q = last_q
            last_q = np.zeros(len(nutrients), dtype=float)
            for i, w in enumerate(w_variable.value):
                v = np.array(data.loc[i, list(nutrients["requirement"])], dtype=float)
                cv = np.array(nutrients["coefficient of variation"], dtype=float)
                last_q = last_q + (w * v * cv) ** 2
            if prev_last_q is not None:
                abs_change = np.max(np.abs(last_q - prev_last_q))
                # print(f'{pass_number} {abs_change} {last_q}')
                if abs_change < 1e-6:
                    return np.array(w_variable.value, dtype=float)
        else:
            return np.array(w_variable.value, dtype=float)
...
def build_check_frame(
    ...
    *,
    ...
    cf = nutrients.copy()
    expected_value = np.zeros(len(cf), dtype=float)
    expected_variance = np.zeros(len(cf), dtype=float)
    for i, w in enumerate(soln):
        if w > 1e-6:
            v = np.array(data.loc[i, list(cf["requirement"])], dtype=float)
            expected_value = expected_value + w * v
            if risk_scale > 0:
                # scaling of single food treated as dependent, so variance scales as a square
                # adding different foods treated as independent, so variances add (instead of standard deviations)
                cv = cf["coefficient of variation"]
                expected_variance = expected_variance + (w * v * cv) ** 2
    cf["expected value"] = expected_value
    if do_assert:
        assert np.max(cf["lower bound"] / cf["expected value"]) < 1 + 1e-5
    if risk_scale > 0:
        cf["modeled risk"] = risk_scale * np.sqrt(expected_variance)
        cf["risk adjusted expectation"] = cf["expected value"] - cf["modeled risk"]
        if do_assert:
            assert (
                np.max(cf["lower bound"] / cf["risk adjusted expectation"]) < 1 + 1e-5
            )
            assert (
                np.max(cf["lower bound"] / cf["risk adjusted expectation"]) > 1 - 1e-5
            )
        del cf["coefficient of variation"]
    return cf
