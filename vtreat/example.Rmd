...
We then prepare a transformed version of the
evaluation/test frame using the treatment plan.  
This is done directly using `vtreat::prepare`.
We could have converted our training data the same way, but as we said
it is more rigorous to use the supplied cross frame (though the nested model
...
And we are now ready to examining the model's out of sample performance.
...
Below we add an indicator if we used the variable in our model and show a few
rows of the `scoreFrame`.
