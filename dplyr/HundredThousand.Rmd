...
Trick question: is a `100,000` cell numeric `data.frame`
large or small? 

In the era of "big data" `100,000` cells is minuscule.  It is plausibly less
than a megabyte of data.  It can fit on a 1986 era `1.44 MB` floppy disk.
...
set up our examples.  A `100,000` row by one column `data.frame` (probably
...
This is slow, but still `r sprintf("%8.1g", ratio)` times faster than using `dplyr`.
...
Some clients have run into issues on `Spark` at around 700 columns.
One step of working around the issue was trying a range of sizes
to try and figure out where the issue was and what the size sensitivity is.
