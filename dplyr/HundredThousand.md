Is 100,000 cells large?
================
Win-Vector LLC
2/12/2018

Trick question: is a `100,000` cell numeric `data.frame` large or small?

In the era of "big data" `100,000` cells is minuscule. It is plausibly less than a megabyte of data. It can fit on a 1986 era `1.44 MB` floppy disk.
...
Let's look at a few examples in [`R`](https://cran.r-project.org). First let's set up our examples. A `100,000` row by one column `data.frame` (probably fairly close the common mental model of a `100,000` cell `data.frame`), and a `100,000` column by one row `data.frame` (frankly bit of an abuse, but large data warehouses with millions of rows and `500` to `1,000` columns are not uncommon).
...
```
    ...
    ## Time difference of 19.07115 secs
...
This is slow, but still 1e+01 times faster than using `dplyr`.
...
Some clients have run into issues on `Spark` at around 700 columns. One step of working around the issue was trying a range of sizes to try and figure out where the issue was and what the size sensitivity is.
