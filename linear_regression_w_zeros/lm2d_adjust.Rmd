...
We're going to use all the data to train our models, but it's not easy to show comparison plots in three dimensions in a way that's legible. So we'll also look at a slices of the data, by holding `u` restricted to a narrow range around a nominal value. Here's a slice with `u` fixed to near  0.5. 
...
For our original problem---post-hoc adjustments of an already established modeling procedure---a GAM adjustment seems to be the best way to adapt our modeling to higher-than-anticipated saturation frequency; however, Tobit adjustment is competitive. If you have the opportunity to design the modeling procedure *de novo*, GAM may not be the best option; though I admit, I didn't spend any time trying to tune the model. If the process you are modeling is well-approximated as linear in its inputs in the non-saturated region, then a thresholded Tobit model appears to be a good choice.
