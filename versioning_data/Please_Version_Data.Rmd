...
Now we see the problem. Our model predicts popcorn sales in the presumed future month of September are going to be double what was seen in the past training month of August. As we don't have the future data yet, we don't immediately known this is wrong. But, without a presumed cause it is suspicious.
...
Note: using the estimated attendance to train (instead of actual) gives a *vastly* inferior R-squared as measured on training data. However, it gives us a model that performs *much* better in the future (which presumably is the actual project goal)!

## Conclusion

The performance of a model on held-out data is only a proxy measure for future model performance. In our example we see that this common proxy idea breaks down when there is a data concept-change between the training and application periods. The fix is: "as of" or bitemporal modeling.
...
Data users should *insist* on bitemporal data for forecasting applications.
