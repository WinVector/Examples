...
gGroups <- paste('g',1:2,sep='')
d <- data.frame(group=sample(gGroups,5,replace=TRUE),
...
for(gName in noiseGroups) {
  gValues <- paste(gName,1:2,sep='.')
  d[[gName]] <- sample(gValues,n,replace=TRUE)
}
...
dTest <- data.frame(group=gGroups,
                    stringsAsFactors=FALSE)
print(dTest)
print(pYgivenRow(dTest,gGroups))
```
...
The idea in this exercise is we are going to have the unknown true probablity P(y|group) as 1-x or x depending on the group label (x, being a variable we vary from 0 to 1) for all groups.  The inner conditoned estimate (called "est") is estimated one of a few ways including:
...
```{r}
plotD2 <- evalModelingStrategy(d,dTest,gGroups,naiveModel,
                               'naive prediction')$plotD
plotD3 <-  evalModelingStrategy(d,dTest,gGroups,jackknifeModel,
                                'jackknife prediction')$plotD
plotD4 <-  evalModelingStrategy(d,dTest,gGroups,jackknifeModel2,
                                'jackknife 2 prediction')$plotD
plotD5 <- c()
tryCatch(
  plotD5 <- evalModelingStrategy(d,dTest,gGroups,splitEstimateExpectedPrediction,
                              'split prediction')$plotD,
  error = function(e) {print(e)}
)

plotDAll <- rbind(plotD2,plotD3,plotD4,plotD5)

ggplot(data=plotDAll,mapping=aes(x=x,
                                 y=expectedMeanSquareDiff,
                                 color=what,fill=what)) +
  geom_line() +
  ggtitle(paste('expected square error as a function of signal strength x,\n n=',n))

ggplot(data=plotDAll,mapping=aes(x=x,
                                 y=expectedBias.g1,
                                 color=what,fill=what)) +
  geom_line() +
  ggtitle(paste('expected bias a function of signal strength x,\n n=',n))
