...
for(ii in seq_len(nrow(ys))) {
  ...
  sPred <- splitEstimateExpectedPrediction(d,dTest)
  sPredExpectedMin <- sPredExpectedMin + min(sPred)*py
  sPredExpectedMean <- sPredExpectedMean + mean(sPred)*py
  sPredExpectedMax <- sPredExpectedMax + max(sPred)*py
}
...
print(sPredExpectedMin)
print(sPredExpectedMean)
print(sPredExpectedMax)
...
plotD5 <- plotD
plotD5$what <- 'split prediction'
plotD5$expectedMin <- as.function(sPredExpectedMin)(plotD$x)
plotD5$expectedMean <- as.function(sPredExpectedMean)(plotD$x)
plotD5$expectedMax <- as.function(sPredExpectedMax)(plotD$x)
...
plotR5 <- plotR
plotR5$what <- 'split prediction'
plotR5$expectedRatioMin <- as.function(sPredExpectedMin)(plotR$x)/plotR1ratio
plotR5$expectedRatioMean <- as.function(sPredExpectedMean)(plotR$x)/plotR1ratio
plotR5$expectedRatioMax <- as.function(sPredExpectedMax)(plotR$x)/plotR1ratio
...
ggplot(data=rbind(plotR3,plotR5),
       mapping=aes(x=x,
                   y=expectedRatioMean,
                   ymin=expectedRatioMin,ymax=expectedRatioMax,
                   color=what,fill=what)) +
...
plotRA <- rbind(plotR2,plotR3,plotR4,plotR5)
