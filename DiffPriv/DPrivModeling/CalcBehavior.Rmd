...
runExperiment <- function(d,experimentName) {
  ...
  # show performance
  plotDAll <- rbind(plotObliv,plotNaive,plotJackknife,plotLNoise,plotAvgNoised)
  print(ggplot(data=plotDAll,mapping=aes(x=x,
                                         y=expectedDeviance,
...
```{r randexpmt}
dRand <- dTest[sample.int(nrow(dTest),10,replace=TRUE),]
runExperiment(dRand,'random design (10 rows)')
```

Experiment where each level occurs exactly once.

```{r randexpmt1}
dOne <- tryFor1design(4,allVars,dTest)
