...
for(ii in seq_len(nrow(ys))) {
  ...
  nPredEstMean <- nPredEstMean + mean(nPred)*py
  ...
  jPredEstMean <- jPredEstMean + mean(jPred)*py
  ...
  jPredEst2Mean <- jPredEst2Mean + mean(jPred2)*py
...
plotD2$estMean <- as.function(nPredEstMean)(plotD$x)
...
plotD3$estMean <- as.function(jPredEstMean)(plotD$x)
...
plotD4$estMean <- as.function(jPredEst2Mean)(plotD$x)
plotD4$estMax <- as.function(jPredEst2Max)(plotD$x)
plotD <- rbind(plotD2,plotD3,plotD4)

ggplot(data=rbind(plotD2,plotD3),mapping=aes(x=x,y=estMean,
                                             ymin=estMin,ymax=estMax,
...
plotR2$ratioMean <- as.function(nPredEstMean)(plotR$x)/plotR1ratio
...
plotR3$ratioMean <- as.function(jPredEstMean)(plotR$x)/plotR1ratio
