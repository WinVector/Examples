...
Attempt at a Q solution. A different "assume no unobserved transitions" solution can be found [here](https://github.com/WinVector/Examples/blob/main/mc_stan/mc_stan.md).
...
model {
  ...
  // cheap-simulate matrix exp
  matrix[n_states, n_states] Q0;
  matrix[n_states, n_states] Q2;
  for (i in 1:n_states) {
    for (j in 1:n_states) {
      if (i != j) {
        Q0[i, j] = 0;
  ...
  for (i in 2:m_examples) {
     if ((pt_idx[i-1] == pt_idx[i]) && (state[i - 1] < n_states)) {
       ...
       t = years[i] - years[i-1];
       // expQ = matrix_exp( t * Q );
       expQ = Q0 + t * Q + pow(t, 2) * Q2 / 2;
       pij = expQ[state[i-1], state[i]];
       target += log(fmax(1e-6, pij)) + pij;
     }
  }
}
"
