...
author: "John Mount, Nina Zume; https://www.win-vector.com"
date: "`r date()`"
...
There are, of course, cases where re-weighting the data by the outcome class can change the order of predictions. For example, we demonstrate a non-trivial reordering effect with two variable (plus intercept) logistic regression [here](rw_example_R.md). In this case we do get different models with different data weightings, but one still must determine which of these different models in in fact better for a given application.
